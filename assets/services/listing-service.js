...
export async function getSellerTrust(userId) {
  const supabase = await getSupabase();
  const { data, error } = await supabase.rpc("get_public_seller_trust", { target_user: userId });
  if (error) throw error;
  return data?.[0] || null;
}
...
export async function uploadPrivateListingImages(listingId, files) {
  ...
  for (let i = 0; i < selected.length; i += 1) {
    ...
    rows.push({ listing_id: listingId, storage_path: path, sort_order: i });
  }
...
export async function submitForReview(listingId) {
  const supabase = await getSupabase();
  const { data, error } = await supabase.rpc("submit_listing_for_review", { target_listing: listingId });
  if (error) throw error;
  return data;
}
...
export async function markListingSold(listingId) {
  const supabase = await getSupabase();
  const { data, error } = await supabase.rpc("mark_listing_sold", { target_listing: listingId });
  if (error) throw error;
  return data;
}
