...
export class AuthService {
  constructor(private readonly prisma: PrismaService, private readonly redis: RedisService, private readonly jwt: JwtService) {}
  ...
  async requestOtp(channel: OtpChannel, rawTarget: string, purpose = 'login', userId?: string) {
    ...
    await this.prisma.otpChallenge.create({ data: { userId, channel, targetHash, codeHash, purpose, expiresAt: new Date(Date.now() + 5 * 60_000) } });
  ...
  private async deliverOtp(channel: OtpChannel, target: string, code: string) {
    ...
    const response = await fetch(url, {
      method: 'POST', headers: { 'content-type': 'application/json', 'authorization': `Bearer ${process.env.OTP_DELIVERY_WEBHOOK_SECRET || ''}` },
      body: JSON.stringify({ channel, target, code, template: 'chovot_login' }), signal: AbortSignal.timeout(8000),
    });
    if (!response.ok) throw new ServiceUnavailableException('OTP_DELIVERY_FAILED');
  }

  async verifyOtp(channel: OtpChannel, rawTarget: string, code: string, purpose = 'login', userId?: string) {
    const target = this.normalize(channel, rawTarget);
    const targetHash = this.digest(target, 'OTP_PEPPER');
    const challenge = await this.prisma.otpChallenge.findFirst({
      where: { targetHash, channel, purpose, userId: userId || null, consumedAt: null, expiresAt: { gt: new Date() } }, orderBy: { createdAt: 'desc' },
    });
  ...
  async loginWithOtp(channel: OtpChannel, rawTarget: string, code: string, meta: { userAgent?: string; ip?: string }) {
    const target = await this.verifyOtp(channel, rawTarget, code, 'login');
    let user = channel === 'EMAIL' ? await this.prisma.user.findUnique({ where: { email: target } }) : await this.prisma.user.findUnique({ where: { phone: target } });
    if (!user) {
      user = await this.prisma.user.create({ data: {
        email: channel === 'EMAIL' ? target : null, phone: channel === 'PHONE' ? target : null,
        profile: { create: {} }, verification: { create: { phoneVerified: channel === 'PHONE' } },
      } });
    } else if (channel === 'PHONE') {
      await this.prisma.sellerVerification.upsert({ where: { userId: user.id }, create: { userId: user.id, phoneVerified: true }, update: { phoneVerified: true } });
    }
  ...
  async issueSession(userId: string, role: UserRole, meta: { userAgent?: string; ip?: string }) {
    ...
    await this.prisma.refreshToken.create({ data: { userId, tokenHash, expiresAt, userAgent: meta.userAgent?.slice(0,500), ipHash } });
  ...
  async refresh(refreshToken: string | undefined, meta: { userAgent?: string; ip?: string }) {
    ...
    if (!row || row.revokedAt || row.expiresAt <= new Date() || row.user.status !== 'ACTIVE') throw new UnauthorizedException('REFRESH_INVALID');
